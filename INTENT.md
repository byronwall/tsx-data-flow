...
This document distills the **durable intents** behind hands-on user feedback,
including the walkthroughs in `docs/feedback/` and the July 2026 product framing
captured while reviewing the Solid client/server migration. The feedback often
arrived as raw, dictated reactions or line-item issues; this is the layer
underneath them — the principles that should decide future design calls,
especially when a specific ticket is ambiguous. When in doubt, optimize for
these.
...
## The job this product is hired to do
...
In practice, that means detecting and explaining unnecessary fallbacks,
...
The desired outcome is orientation, not merely a report. Given an unfamiliar
changed file, the user should be able to identify the most consequential newly
introduced complexity, understand its path through the code, and locate the
responsible source within one minute.

This is a local, disposable review instrument. A user runs it, understands the
code, perhaps gives the evidence to an agent, and moves on. Results, entity IDs,
and links do not need to remain durable across analyses, commits, machines, or
time.
...
The code map is the primary working surface: all analysis hangs from it. It
should act as ambient information, showing where the important problems are
before the user proactively inspects every variable. Selecting a value or
finding then explains its steps, fallbacks, transformations, hops, reach, and
locations without taking the user away from the source.
...
props. Remaining reports are supporting triage lenses, not automatically equal
products. Migrate, merge, defer, or delete each according to demonstrated value.
...
Markdown is a human-and-agent handoff surface. It should be dense, grounded, and
easy to paste into an agent prompt: what happened, where it happened, why it
matters, the relevant path, and a concise next action. Avoid generic suggested-fix
prose and repetitive "words, words, words."

JSON is an implementation and automation surface. It should be structured and
reliable, but its current accidental shape is not a public compatibility
promise. Preserve exact compatibility only when a real external consumer is
identified.
