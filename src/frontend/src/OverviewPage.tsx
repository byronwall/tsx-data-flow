...
export function OverviewPage(props: { location: URL; navigate: Navigate }) {
  ...
  return <Shell context={response.error ? "" : report()?.workspace.displayRoot ?? ""} tabs={() => <ReportTabs active={null} />}><Show when={!response.error} fallback={<p class="error" role="alert">{response.error?.message ?? "Unable to load the workspace."}</p>}><Show when={!response.loading} fallback={<p class="meta">Loading analysis…</p>}>
    ...
    <Show when={report()}>{(workspace) => <><OverviewControls state={state()} navigate={props.navigate} /><OverviewResults report={workspace()} state={state()} rows={rows()} pageRows={pageRows()} typeCounts={entryTypeCountsByFile(workspace())} /></>}</Show>
  </Show></Show></Shell>;
}
