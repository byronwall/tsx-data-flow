...
export function routeTotalityEdgePathForLayout(edge: RouteTotalityLayoutEdge): string {
  ...
  if (edge.from === edge.to) {
    return `M ${from.x} ${from.y} C ${from.x + 48} ${from.y - 42}, ${from.x + 48} ${from.y + 42}, ${from.x} ${from.y}`;
  }

  const start = connectionPoint(edge.fromNode, to);
  const end = connectionPoint(edge.toNode, from);
  const offset = (edge.parallelIndex - (edge.parallelCount - 1) / 2) * 14;
  ...
  const bend = Math.max(28, Math.abs(dy) * 0.42);
