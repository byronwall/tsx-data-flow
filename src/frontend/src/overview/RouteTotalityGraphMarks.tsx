...
export function TotalityEdge(props: {
  ...
  const interactionHidden = createMemo(() => props.hidden);
  return <g ref={props.onRegister} data-route-selection={props.selection.graphId} class={`route-totality-edge-family-${props.edge.family}`} classList={{ selected: props.selected, "emphasis-active": props.active, "emphasis-frontier": props.frontier, "emphasis-dimmed": props.emphasisActive && !props.active && !props.frontier, "isolation-hidden": props.hidden, "interaction-hidden": interactionHidden() }} role={interactionHidden() ? undefined : "button"} tabIndex={interactionHidden() ? undefined : 0} aria-hidden={interactionHidden() ? "true" : undefined} aria-pressed={interactionHidden() ? undefined : props.selected} aria-label={interactionHidden() ? undefined : `Select ${props.edge.family} edge ${routeTotalityEdgeLabel(props.edge)}${props.active ? " · in emphasized reach" : props.frontier ? " · partial frontier" : ""}`} onPointerDown={(event) => { if (!interactionHidden()) event.stopPropagation(); }} onClick={(event) => { if (interactionHidden()) return; event.stopPropagation(); props.onSelect(props.selection); }} onKeyDown={(event) => { if (!interactionHidden()) selectOnKey(event, props.selection, props.onSelect); }}>
...
export function TotalityNode(props: {
  ...
  const interactionHidden = createMemo(() => props.hidden);
  return <g ref={props.onRegister} data-route-selection={props.selection.graphId} class={`route-totality-node-kind-${props.node.kind}`} classList={{ selected: props.selected, "emphasis-active": props.active, "emphasis-frontier": props.frontier, "emphasis-dimmed": props.emphasisActive && !props.active && !props.frontier, "isolation-hidden": props.hidden, "interaction-hidden": interactionHidden() }} transform={`translate(${props.node.x} ${props.node.y})`} role={interactionHidden() ? undefined : "button"} tabIndex={interactionHidden() ? undefined : 0} aria-hidden={interactionHidden() ? "true" : undefined} aria-pressed={interactionHidden() ? undefined : props.selected} aria-label={interactionHidden() ? undefined : `Select ${routeTotalityNodeKindLabel(props.node.kind)} ${label()}${props.active ? " · in emphasized reach" : props.frontier ? " · partial frontier" : ""}${props.findings.count ? ` · ${props.findings.count} exact finding${props.findings.count === 1 ? "" : "s"}` : ""}`} onPointerDown={(event) => { if (!interactionHidden()) event.stopPropagation(); }} onClick={(event) => { if (interactionHidden()) return; event.stopPropagation(); props.onSelect(props.selection); }} onKeyDown={(event) => { if (!interactionHidden()) selectOnKey(event, props.selection, props.onSelect); }}>
