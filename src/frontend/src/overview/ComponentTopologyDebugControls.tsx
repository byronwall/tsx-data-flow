...
export function ComponentTopologyDebugControls(props: {
  ...
  return <aside class="component-topology-debug" aria-label="Topology layout debug controls">
    ...
    <div class="component-topology-debug-actions">
      <button type="button" aria-pressed={props.editingPositions} onClick={() => props.onToggleEditing()}>{props.editingPositions ? "Done editing" : "Edit positions"}</button>
    ...
    <DebugRange label="Link distance" value={props.settings.targetLinkDistance} minimum={40} maximum={240} step={2} onInput={(value) => props.onSetting("targetLinkDistance", value)} />
    ...
    <DebugRange label="Collision" value={props.settings.collisionStrength} minimum={.2} maximum={1.8} step={.1} onInput={(value) => props.onSetting("collisionStrength", value)} />
    ...
    <small>{props.editingPositions ? "Drag nodes to reposition" : `${props.manualMoveCount} manual ${props.manualMoveCount === 1 ? "move" : "moves"} · ${props.settings.separationPasses} separation ${props.settings.separationPasses === 1 ? "pass" : "passes"}`}{props.forcesVisible ? " · arrows show exact next-tick displacement (1:1)" : ""}</small>
  </aside>;
}
