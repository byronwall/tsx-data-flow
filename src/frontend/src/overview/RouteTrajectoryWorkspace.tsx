...
export function RouteTrajectoryWorkspace(props: { detail: RouteDataDetail; state: TrajectoryUrlState; onState: (patch: Partial<TrajectoryUrlState>, push?: boolean) => void; onCloseTransient: (active: boolean) => void }) {
  ...
  const openEvidence = (id: string) => { setSourceEvidenceId(id); props.onCloseTransient(true); };
  const closeEvidence = () => { setSourceEvidenceId(null); props.onCloseTransient(false); };
  ...
  const sourceEvidence = createMemo(() => props.detail.evidence.find((item) => item.id === sourceEvidenceId()) ?? null);
  ...
  return <div class="route-trajectory-workspace" classList={{ "route-flow-open": props.state.view === "context" }}>
    <main>
      <Show when={props.state.view === "context"} fallback={<>
      ...
      </>}><RouteFlowGraph detail={props.detail} onOpenEvidence={openTrajectory} /></Show>
